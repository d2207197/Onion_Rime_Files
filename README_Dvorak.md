...
| 模式 | 說明 | 如何進入 |
|------|------|---------|
| **編輯模式** | 輸入注音、移動游標、修改內容 | `Tab`、`Shift+Tab`、`Enter` 確認後 |
| **選字模式** | 候選選單出現，用 ↑↓ 選字 | `↓` 或自動觸發 |
...
|------|------|---------|
| `Tab` | 往後跳一個字 | 選字 → **編輯** |
| `Shift+Tab` | 往前跳一個字 | 選字 → **編輯** |
| `↓` | 展開候選 / 下一個候選 | 編輯 → **選字** |
| `↑` | 上一個候選 | |
| `Enter` 或 `空白` | 確認選字 | 選字 → **編輯** |
| `BackSpace` | 刪除一個字（整組注音） | |
| `← →` | 在字之間移動（效果類似 Shift+Tab / Tab） | |

> **推薦以 Tab 為主**：Tab 同時完成「離開選字模式」和「跳到下一個字」，
> 從頭往右跳，非常適合逐字確認的流程。
...
```
1. 打完整句注音，Rime 自動選字
2. Tab Tab Tab ...                  ← 連按 Tab，一路跳過正確的字
3. ↑ ↓                             ← 遇到錯的字，用上下選正確的
4. Enter                            ← 確認（一次確認到目前為止的所有字）
5. Tab Tab Tab ...                  ← 繼續往後跳過正確的字
6. 重複 3-5 直到全部正確
7. Enter                            ← 最後上屏
```

> Tab 連按時不需要每個字都按 Enter。多按幾次 Tab 跳過正確的字，
> 只在需要改字或想確認一段時才按 Enter。

### 移動與選字按鍵

| 按鍵 | 功能 |
|------|------|
| `Tab` | 往後跳一個字（同時離開選字模式）。連按可快速跳過正確的字 |
| `Shift+Tab` | 往前跳一個字 |
| `← →` | 在字之間移動（效果類似 Shift+Tab / Tab） |
| `↑ ↓` | 在候選中選擇不同的字或詞 |
| `Enter` 或 `空白` | 確認選字（確認到目前游標位置為止的所有字） |
| `BackSpace` | 刪除一個字（整組注音） |
...
Rime 有兩種模式，按鍵在不同模式下行為不同：

- **編輯模式**（composing）：輸入注音、移動游標、修改內容
- **選字模式**（has_menu）：候選選單出現，選字翻頁
...
| `→` | 前進一個字 | 下一頁 | |
| `Tab` | 下一節（往後跳） | — | **→ 回到編輯模式** |
| `Shift+Tab` | 上一節（往前跳） | — | **→ 回到編輯模式** |
| `Enter` | 上屏 | 確認選字 | **→ 回到編輯模式** |
| `Shift+←` | 左移一個注音 | — | |
| `Shift+→` | 右移一個注音 | — | |
| `Shift+↑` | — | 上一頁 | |
| `Shift+↓` | — | 下一頁 | |
