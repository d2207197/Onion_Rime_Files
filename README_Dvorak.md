# 洋蔥注音 Plus Dvorak 版 使用指南

本方案基於[洋蔥注音 Plus 版](https://github.com/oniondelta/Onion_Rime_Files)，
針對 **Dvorak 鍵盤 layout** 調整，讓使用者用 Dvorak 的物理鍵位輸入大千式注音。

## 安裝

將以下檔案放入 Rime 使用者目錄（macOS: `~/Library/Rime/`）：

- `bopomo_onionplus_dvorak.schema.yaml` — Dvorak 版方案定義
- `element_bopomo_dvorak.yaml` — Dvorak 版按鍵設定
- 其餘所有檔案與原版洋蔥注音 Plus 共用（字典、lua、opencc 等）

在 `default.custom.yaml` 中加入：

```yaml
patch:
  schema_list:
    - schema: bopomo_onionplus_dvorak
```

重新部署鼠鬚管即可使用。

---

## 設計原則

1. **右手九宮格選字**：GCR / HTN / MWV 九鍵專用選字，共 9 候選
2. **Emacs 風格游標**：F=forward、B=backward、D=delete，圍繞九宮格
3. **助記鍵**：所有功能鍵都有助記（vi/Emacs/英文縮寫），不需死記
4. **一個功能一個鍵**：不重複綁定，簡潔明確

---

## 注音鍵位對照

每個物理按鍵對應的注音符號與 QWERTY 大千式完全相同（依物理位置）：

```
QWERTY 鍵盤排列（大千式注音原版）：
┌─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┐
│  q  │  w  │  e  │  r  │  t  │  y  │  u  │  i  │  o  │  p  │  -  │
│  ㄆ │  ㄊ │  ㄍ │  ㄐ │  ㄔ │  ㄒ │  ㄧ │  ㄛ │  ㄟ │  ㄣ │  ㄦ │
├─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┤
│  a  │  s  │  d  │  f  │  g  │  h  │  j  │  k  │  l  │  ;  │     │
│  ㄇ │  ㄋ │  ㄎ │  ㄑ │  ㄕ │  ㄗ │  ㄨ │  ㄜ │  ㄠ │  ㄤ │     │
├─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┤
│  z  │  x  │  c  │  v  │  b  │  n  │  m  │  ,  │  .  │  /  │     │
│  ㄈ │  ㄌ │  ㄏ │  ㄒ │  ㄖ │  ㄙ │  ㄩ │  ㄝ │  ㄡ │  ㄥ │     │
└─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┘

Dvorak 鍵盤排列（同物理位置，同注音符號）：
┌─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┐
│  '  │  ,  │  .  │  p  │  y  │  f  │  g  │  c  │  r  │  l  │  [  │
│  ㄆ │  ㄊ │  ㄍ │  ㄐ │  ㄔ │  ㄒ │  ㄧ │  ㄛ │  ㄟ │  ㄣ │  ㄦ │
├─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┤
│  a  │  o  │  e  │  u  │  i  │  d  │  h  │  t  │  n  │  s  │     │
│  ㄇ │  ㄋ │  ㄎ │  ㄑ │  ㄕ │  ㄗ │  ㄨ │  ㄜ │  ㄠ │  ㄤ │     │
├─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┼─────┤
│  ;  │  q  │  j  │  k  │  x  │  b  │  m  │  w  │  v  │  z  │     │
│  ㄈ │  ㄌ │  ㄏ │  ㄒ │  ㄖ │  ㄙ │  ㄩ │  ㄝ │  ㄡ │  ㄥ │     │
└─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┘

數字列（兩者相同）：
┌─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┐
│  1  │  2  │  3  │  4  │  5  │  6  │  7  │  8  │  9  │  0  │
│  ㄅ │  ㄉ │  ˇ  │  ˋ  │  ㄓ │  ˊ  │  ˙  │  ㄚ │  ㄞ │  ㄢ │
└─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┘

聲調：空白鍵 = ˉ（一聲）、6 = ˊ、3 = ˇ、4 = ˋ、7 = ˙
```

---

## 選字操作

候選字選單出現後，共 9 個候選字，使用右手九宮格選字：

```
┌──────────┬──────────┬──────────┐
│ G = 候選1│ C = 候選2│ R = 候選3│
├──────────┼──────────┼──────────┤
│ H = 候選4│ T = 候選5│ N = 候選6│
├──────────┼──────────┼──────────┤
│ M = 候選7│ W = 候選8│ V = 候選9│
└──────────┴──────────┴──────────┘
```

- **Enter** = 確認第 1 候選（平時使用）
- **Shift+G** = 選第 1 候選（回頭重選字時方便）

> QWERTY 原版使用左右手各 8 鍵（共 16 鍵）選 8 個候選。
> Dvorak 版改為右手九宮格 9 鍵選 9 個候選，更直覺且候選更多。

### 翻頁（vi 風格）

| 操作 | 按鍵 | 助記 |
|------|:----:|------|
| 上一頁 | `Shift+K` | vi k=up |
| 下一頁 | `Shift+J` | vi j=down |
| 上一頁 | `←` | 選字狀態時 |
| 下一頁 | `→` | 選字狀態時 |

---

## 快捷鍵

### Shift+鍵 功能總覽

```
Dvorak 右手：                              Dvorak 左手：
  F(fwd) [G=1  C=2  R=3]                    P(注音文) ...
  D(del) [H=4  T=5  N=6]                    O(orig)  E(end)  A(home)
  B(bwd) [M=7  W=8  V=9]  Z(BkSp)          Q(quit)  J(pgdn) K(pgup)
```

### 右手 — Emacs 游標與刪除（composing 時）

| 按鍵 | 功能 | 助記 |
|------|------|------|
| `Shift+F` | 右移一個字 | Emacs C-**f** = **f**orward |
| `Shift+B` | 左移回退一個字 | Emacs C-**b** = **b**ackward |
| `Shift+D` | 向右刪除 (Delete) | Emacs C-**d** = **d**elete |
| `Shift+Z` | 刪除整組注音 (BackSpace) | 九宮格右下，快速刪字 |

### 左手 — 導航

| 按鍵 | 功能 | 助記 |
|------|------|------|
| `Shift+A` | Home（跳到開頭） | Emacs C-**a** |
| `Shift+E` | End（跳到結尾） | Emacs C-**e** |
| `Shift+J` | Page Down | vi **j**=down |
| `Shift+K` | Page Up | vi **k**=up |

### 左手 — 操作與上屏

| 按鍵 | 功能 | 助記 |
|------|------|------|
| `Shift+Q` | Escape（清除輸入） | **Q**=Quit |
| `Shift+O` | 上屏原始輸入 | **O**=Original |
| `Shift+P` | 上屏注音文 | **P**=Phonetic |

### 刪除操作

| 按鍵 | 效果 | 說明 |
|------|------|------|
| `BackSpace` | 刪除整組注音（一個字） | Rime 共用設定 |
| `Shift+BackSpace` | 刪除一個注音字符 | 精細刪除 |
| `\` | 刪除一個注音字符 | 選字狀態時可用 |
| `Shift+D` | 向右刪除 (Delete) | Emacs C-d |
| `Shift+Z` | 刪除整組注音 (BackSpace) | 右手小指，九宮格旁 |
| `Fn+Shift+Delete` | 刪除記憶詞 | 先將游標移到該候選上再按。Rime 內建功能 |

### 切分移位（方向鍵）

| 功能 | 按鍵 |
|------|------|
| 左移一個注音 | `↑`、`Shift+←` |
| 右移一個注音 | `Shift+→` |
| 右移一整個字 | `→` |
| 回到輸入列 | `Tab`（選字狀態時） |

> **Shift+F/B**（字級移動）和**方向鍵**（注音級移動）搭配使用，
> 可以快速定位到要修改的位置。

### 中英文與模式切換

| 功能 | 按鍵 |
|------|------|
| 切換中/英文 | `Control+Shift+2` 或 `Shift+空白` |
| 切換方案 | `Control+~` 或 `F4` |
| 切換全/半形 | `Control+Shift+3` |
| 切換繁/簡 | `Control+Shift+5` |
| 切換標點模式 | `Control+Shift+4` |

---

## 外掛語言與特殊功能

洋蔥 Plus 版內建多種外掛語言輸入，在注音模式下輸入特定前綴啟動：

| 功能 | 前綴 | 提示 | 說明 |
|------|:----:|------|------|
| 英文 | `3` | 《Easy》 | 英漢字典查詞 |
| 拉丁字母 | `4` | 《拉》 | 含重音符號的拉丁字母 |
| 日文 | `6` | 《日》 | 平假名、片假名、漢字 |
| 俄文 | `66` | 《俄》 | 西里爾字母 |
| 韓文 | `7` | 《韓》 | 韓語 HNC |
| 希臘文 | `77` | 《希》 | 希臘字母 |
| 全形 | `][` | 《全》 | 全形英數字 |
| 注音文 | `';` | 《注音文》 | 輸出注音符號本身 |
| emoji | `` ` `` | 《emoji集》 | 表情符號系列 |
| 倉頡反查 | `]` + 倉頡碼 | 《查》 | 用倉頡碼反查注音（Dvorak `]` = QWERTY `=` 物理位置）|
| 數學/功能 | `'/` | — | 日期時間、計算機等 lua 功能 |

---

## 標點符號

Dvorak 版覆蓋了部分標點鍵，讓常用標點維持 QWERTY 的**物理位置**。

### 直接輸入

| 按鍵 | 輸出 | 說明 |
|:----:|:----:|------|
| `<` (Shift+,) | ， | 逗號 |
| `>` (Shift+.) | 。 | 句號 |
| `?` (Shift+/) | ？ | 問號 |
| `:` (Shift+;) | ： | 冒號 |
| `"` (Shift+') | ； | 分號 |
| `~` | ～ | 波浪號 |
| `!` | ！ | 驚嘆號 |
| `(` | （ | 左括號 |
| `)` | ） | 右括號 |
| `{` | 〔 | 左方括號 |
| `}` | 〕 | 右方括號 |

### 多選標點（Dvorak 專屬覆蓋）

| Dvorak 鍵 | 首選 | QWERTY 物理位置 | 說明 |
|:---------:|:----:|:---------------:|------|
| `-` | 、 | `'` (Enter 旁) | **頓號**系列 |
| `/` | 「 | `[` | **左引號**系列（含〈〔等） |
| `=` | 」 | `]` | **右引號**系列（含〉〕等） |
| `]` | ＝ | `=` | **等號**系列（含々〃等） |

### 其他標點（維持 punct_bopomo 原設定）

| 按鍵 | 首選 | 說明 |
|:----:|:----:|------|
| `'` | 、 | 頓號（僅非輸入狀態，因 `'` 同時是 ㄆ） |
| `[` | 「 | 左引號（僅非輸入狀態，因 `[` 同時是 ㄦ） |
| `\` | 刪除 | composing 時是注音字符刪除 / 非 composing 時是反斜線 |

---

## 檔案結構

```
Dvorak 專屬檔案（本 fork 新增）：
├── rimefiles/bopomo_onionplus_dvorak.schema.yaml  # 方案定義
├── rimefiles/element_bopomo_dvorak.yaml            # 按鍵設定
└── rimefiles/各方案default.custom/
    └── 注音洋蔥plus版Dvorak_custom/
        └── default.custom.yaml                     # 預設設定

共用檔案（來自 upstream，不需修改）：
├── rimefiles/bopomo_onionplus.extended.dict.yaml   # 字典
├── rimefiles/punct_bopomo.yaml                     # 標點定義
├── rimefiles/lua/                                  # Lua 腳本
├── rimefiles/opencc/                               # OpenCC 轉換
└── ...（其餘所有 rimefiles/ 下的檔案）
```

---

## 與 QWERTY 原版的差異摘要

| 項目 | QWERTY 原版 | Dvorak 版 |
|------|-------------|-----------|
| 注音鍵位 | 大千式 QWERTY | 大千式 Dvorak（同物理位置） |
| 選字 | 8 候選，左右手各 8 鍵 | 9 候選，右手九宮格 9 鍵 |
| 選字標籤 | `𝟷𝚀𝚈` 等 (Unicode) | `G` `C` `R` 等 (ASCII) |
| 游標移動 | 方向鍵為主 | 方向鍵 + Shift+F/B（Emacs） |
| 刪除 | Shift+P/T（物理位置） | Shift+D（Emacs C-d）+ Shift+\ |
| 翻頁 | Shift+Z / Shift+空白 | Shift+J/K（vi 風格） |
| 導航 | Shift+V/B（物理位置） | Shift+A/E（Emacs C-a/C-e） |
| 取消 | Shift+G（物理位置） | Shift+Q（Quit） |
| 上屏 | Shift+O/F/R（物理位置） | Shift+O/P（Original/Phonetic） |
| 頓號 | `'` (Enter 旁) | `-` (Enter 旁，同物理位置) |
| 引號 | `[` `]` | `/` `=`（同物理位置） |
| 字典/prism | bopomo_onionplus | 字典共用，prism 獨立編譯 |
| 設定檔 | element_bopomo | element_bopomo_dvorak |
